...
class CenternetHead():
    ...
    def _complex_upsample(self,fm,output_dim, factor=2,scope='upsample'):
        with tf.variable_scope(scope):
            ...
            x = tf.keras.layers.UpSampling2D(data_format='channels_last', interpolation='bilinear',
                                             size=(factor, factor))(x)


            y = slim.conv2d_transpose(y,
                                       output_dim//2,
                                       [3, 3],
                                       stride=2,
                                       padding='SAME',
