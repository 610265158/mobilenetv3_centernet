...
def ssd_loss(reg_predict,cls_predict,reg_label,cls_label_,which_loss='focal_loss'):


    cls_label=cls_label_[:,0,:]

    anchor_states = cls_label_[:,1,:]
...
def localization_loss(predictions, targets, weights,sigma=9):
    ...
    indices = tf.where(tf.equal(weights, 1))
...
def focal_loss(prediction_tensor, target_tensor, weights=None, alpha=0.25, gamma=2):
    ...
    indices = tf.where(tf.greater(weights, 0.))
    target_tensor = tf.gather_nd(target_tensor, indices)
    prediction_tensor = tf.gather_nd(prediction_tensor, indices)
