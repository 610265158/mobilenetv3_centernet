...
def get_all_anchors_fpn(strides=None, sizes=None,scales=None,max_size=[640,640]):
    """
    Returns:
    ...
    if sizes is None:
        sizes = cfg.ANCHOR.ANCHOR_SIZES
    if scales is None:
        scales = cfg.ANCHOR.ANCHOR_SCALES
    if max_size is None:
    ...
    for stride, size in zip(strides, sizes):
        sizes_ = size * np.array(scales)
        foa = get_all_anchors(stride=stride, sizes=sizes_,max_size=max_size)
