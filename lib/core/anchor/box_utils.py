...
###becaurefull , the decode use it as one
...
def decode_fix(codes, anchors,anchors_decode):
    """Decode relative codes to boxes.
    Arguments:
        codes: a float tensor with shape [N, 4],
            anchor-encoded boxes of the format [tx1, ty, tx2, ty2].
    ...
    with tf.name_scope('decode_predictions'):
        ...
        decodes=codes*anchors_decode+anchors
...
def batch_decode_fix(box_encodings, anchors,anchors_decode):
    """Decodes a batch of box encodings with respect to the anchors.
    ...
    #     [batch_size, 1, 1]
    # )  # shape [batch_size, num_anchors, 4]
    #tiled_anchor_boxes=anchors

    decoded_boxes = decode_fix(
        tf.reshape(box_encodings, [-1, 4]),
        anchors,
        anchors_decode
    )  # shape [batch_size * num_anchors, 4]
