...
class CellAnchor():
    ...
    @classmethod
    def generate_cell_anchor(self, base_size=16, ratios=[0.5, 1., 2.], scales=2 ** np.arange(3, 6),
                             rect=cfg.ANCHOR.rect):
...
class Anchor():
    ...
    def get_all_anchors_fpn(self):
        ...
        for stride, size in zip(strides, sizes):
            sizes_ = size * np.array(self.scales)
            foa = self.get_all_anchors(stride=stride, sizes=sizes_)
            foas.append(foa)
    ...
    def get_decode_anchor(self):
        '''

        :return: the anchor decode [w,h,w,h]

        the right way to decode the bbox is   res *[w,h,w,h]+ raw_anchor,  which means   anchors_bias +anchors
        '''
        anchor_widths = self.anchors[:, 2] - self.anchors[:, 0]
        anchor_heights = self.anchors[:, 3] - self.anchors[:, 1]
        tmp_anchor_details = np.stack([anchor_widths, anchor_heights, anchor_widths, anchor_heights], axis=1)

        return tmp_anchor_details
...
if __name__ == '__main__':
    ...
    # for i in range(10000,anchors.shape[0]):
    #     box=anchors[i]
    #     print(box[2]-box[0])
    ...
    a, b = anchor_maker.produce_target(np.array([[34., 396., 58., 508.], [20, 140, 50, 160]]), np.array([1, 1]))
