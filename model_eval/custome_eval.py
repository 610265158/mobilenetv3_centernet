...
ap.add_argument("--annFile", required=True, default='./model_eval/DatasetTest_cocoStyle.json', help="coco style json")
ap.add_argument("--imgDir", required=True, default='', help="coco style json")
...
def predict_box():
    ...
    for img_id in tqdm(imgIds):
        ...
        fname=os.path.join(IMAGE_DIR,cocoGt.loadImgs(img_id)[0]['file_name'])
        ...
        if args.is_show:
            for i in range(detect_res.shape[0]):
                ...
                str_draw = '%s:%.2f' %(coco_map[int(one_box[5])][1],one_box[4])

                cv2.rectangle(image_show, (int(one_box[0]), int(one_box[1])), (int(one_box[2]), int(one_box[3])),
                              (0, 255, 0), 2)
                cv2.putText(image_show, str_draw, (int(one_box[0]), int(one_box[1])), cv2.FONT_HERSHEY_SIMPLEX, 1,
                            (255, 0, 255), 2)
        ...
        for i in range(detect_res.shape[0]):
            ...
            res_coco.append({
                'bbox': box,
                'category_id': coco_map[int(one_box[5])][0],
                'image_id': img_id,
                'score': one_box[4]
            })
