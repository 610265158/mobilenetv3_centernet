...
def detect_face(img, shrink):
    ...
    if not MULTISCALETEST:
        detections = face_detector(img, score_threshold=0.05, input_shape=(args.input_shape, args.input_shape))
    else:
        INPUT_SHAPE = (img.shape[0], img.shape[1])
        detections = face_detector(img, score_threshold=0.05, input_shape=INPUT_SHAPE)
